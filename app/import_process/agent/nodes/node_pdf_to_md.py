...
def step_3_download_and_extract(zip_url: str, output_dir_obj: Path, pdf_stem: str) -> str:
    ...
    # 2. 清理旧解压目录（同名）并解压ZIP包（两次解压的内容可能不一样，避免旧文件干扰，为每个PDF创建专属目录）
    ...
    # 核心解压操作，保留原目录结构
    # zipfile 为Python 内置模块，用于处理ZIP文件
    with zipfile.ZipFile(zip_save_path, 'r') as zip_file_obj: # 创建ZipFile对象，只能读取解压
        zip_file_obj.extractall(extract_target_dir) # 执行解压，解压到 extract_target_dir
    ...
    for md_file in md_file_list:
        if md_file.stem == pdf_stem:
            ...
            break
    # 优先级2：MinerU默认生成的full.md（不区分大小写），旧版本（默认名称为 full.md）兼容
    if not target_md_file:
        for md_file in md_file_list:
            if md_file.name.lower() == "full.md":
                ...
                break
    # 优先级3：兜底取第一个MD文件，完全兜底（MinerU 内部生成其他的不知名字的 markdown 文件）
    if not target_md_file:
